...
## 
## shpfilex <- "NEIGHBORHOODS_WGS84_2.shp"
...
## gives "MULTIPOLYGON" format
coords <- readOGR("../includes/ISR_adm/ISR_adm1.shp")
## also works
coords  ## great, gives lat/long
## but how to get just the points for the golan?
## TODO shapGolan <- polygon[]
...
dim(states)
dim(Golan) 
Golan  ## the coordinates are one of the things shown,
## how can I access just that piece?
typeof(Golan)
## access s4 slots:
# http://stackoverflow.com/questions/13099780/how-to-access-the-slots-of-an-s4-object-in-r

# Golan@coords  # wrong
# coords@Golan  # wrong
slotNames(Golan)
# coords@polygons(Golan)  # Error: attempt to apply non-function
slot(Golan, coords)
str(Golan)
...
golanframe$piece <- 1
golanframe$id    <- 1878
#golanframe   # look!  success

```
## Great! 
Now that I have extracted the shape of the Golan from a reputable source,
I still need one more region, 
the Haifa suburbs or Qrayot Haifa.

This shape I had to draw by hand by looking at google maps and using the QGIS software.
Here I extract these points using the methodology derived above.
...
## combine with package version



#coordinates(gpoly@coords)

v <- extract(gpoly, small = TRUE)
